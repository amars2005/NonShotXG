...
\textbf{Cross-attention} extends this idea: queries come from one sequence and keys and values from another. For Version 3, this means the ball position can directly query the player context: given where all the players are, how should the threat estimate for this ball position be adjusted?
...
Online, I was able to find a large dataset, including lots of data from thousands of football matches, across many different leagues and competitions. This dataset is open-source, and can be found at \url{https://github.com/statsbomb/open-data}. As it is open-source, it is perfect for us to use to create our model, as we can use the data for any means we want.

\subsection{What Data Does It Include}

This dataset contains data from over a thousand matches, all saved as JSON files. These files, each one representing a game, include the line-ups of each team, accurate to the second data of where the ball is at all times, and the start and end position of the ball on every play, whether that's a pass, a dribble, or a shot. This means I will be able to track the position of the ball at all times, and see from which positions a goal is most likely to occur. However this dataset is not fully comprehensive, as it doesn't include other valuable information, such as the position of every single player at all times, which can have a large impact on xG, as shooting at an empty net is very different to shooting at one with a goalkeeper in front. This means that while we can produce a very accurate model, we can't account for every single variable possible.

From Version 2 onwards, the model also makes use of StatsBomb's \textbf{360 data}. For a subset of matches in the dataset, StatsBomb provides freeze-frame snapshots: at the moment of each tracked event, the exact positions of all visible players on the pitch are recorded. This data is available for 323 matches and enables the model to see not just where the ball is, but where every player is at the exact moment of each action , a significant upgrade over the original event data alone.

\subsection{Compilation to Python}

To predict the likelihood of scoring, I will be using Python, and some of its modules to predict the likelihood of scoring. This means I will need to get the data from the JSON files provided, extract all necessary information, and process it into either Python classes or a spreadsheet CSV file.
...
Before I process my data, I wanted to check whether these features correlate highly with one another. If there are highly correlated variables, not only does this bring more computation than is necessary, the correlated variables don't provide enough necessary information to compensate for this extra training. To decide this information, I have listed out my variables, and produced a feature correlation graph, to visualise how much each feature matches the others from a scale from -1 to 1, and you can see this map here.
...
For Versions 2 and 3, the training labels are soft floating-point values produced by temporal discounting, not the binary 0/1 labels that ROC AUC expects. Labels are binarised before computing the metric: any event with a label above zero (i.e., any event in a goal-scoring chain) is treated as positive. This preserves the intended semantics of the label while making the metric computable.
...
After all of this data is processed, we can start to think about how to train a model to predict the likelihood of scoring. In this iteration of training, I have decided to go with a Random Forest Model, which generates many different decision trees, and combines their output to generate a single result. In order to do this, we have to first split the data into training data, and validation data, and decide on metrics to effectively infer how accurate our model is to real life.
...
To be able to infer as well how well the model performs, I will be using ROC AUC, as well as the Brier Score. ROC plots the True Positive rate against the False Positive Rate on a graph, where the ROC AUC score is given by the area under this graph. A ROC AUC score of 1 shows a perfect model, whereas if it is 0.5, our model is just making predictions at random. A Brier score on the other hand is used to evaluate the accuracy of predictions for binary outcomes, and is calculated by taking the mean squared difference between predicted probabilities and actual outcomes, ranging from the best possible at 0 and the worst possible at 1.
...
Across all six panels the dominant story is similarity: the Version 2 scenario grids look almost like six copies of the same heatmap with dots placed differently on top. The CNN processes player positions as a continuous density field, and that representation simply cannot discriminate between defensive formations that have equal density but different tactical shape.

\subsection{Limitations and Motivation for Version 3}

Version 2's core limitation is structural: player positions are encoded as image channels, and the CNN must infer their relationships implicitly from the resulting density patterns. A CNN is well-suited to detecting local spatial patterns, but it cannot directly represent that a specific defender is standing between ball and goal, or that two attackers are making runs behind the defensive line. The scenario comparison above illustrates this directly. Encoding player positions as tokens and attending over them explicitly is the natural fix.
...
Only Stage 2 is trained; Stage 1 weights are frozen. Stage 2 therefore starts from a strong positional baseline and learns only the residual correction based on player context, which keeps training tractable on the 323-match freeze-frame dataset.
...
The two biggest gains came from introducing player positional context (the 360 data) and adopting a deep learning architecture capable of learning non-linear spatial patterns. The standardised four-metric framework makes these improvements directly comparable across all three versions.

\subsection{Limitations and Threats to Validity}

The comparison has four caveats worth noting.

The Version 1 and Version 2/3 test sets are not the same population. Version 1 is evaluated on all events from 520 matches; Versions 2 and 3 use 140,697 events from 49 freeze-frame matches. This reflects the data available to each model, but it means the Version 1 $\to$ Version 2 jump cannot be attributed to architecture alone; the freeze-frame subset may differ systematically from the full dataset.

With only 49 test matches, the Version 2 and Version 3 metric estimates carry more variance than Version 1's, and a different random seed could shift the test set noticeably.

The Spearman $\rho$ values for Versions 2 and 3 are computed against soft discounted labels, which are an approximation of true event contribution. The discount factor $\gamma = 0.9$ is a modelling choice; a different value would produce different $\rho$ values.

All three models used a single random seed (42) for both training and the train/test split. Version 3's $\Delta\rho$ could plausibly be more positive or negative under a different partition of the 323 freeze-frame matches.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{frontend_screenshot.png}
    \caption{Interactive xT web frontend: ball carrier placed at approximately $(83, 40)$, two
             outfield teammates at the edge of the penalty area, two opponents inside the box,
             and an opponent goalkeeper. The model returns a weighted xT of 0.372 with a
             per-action breakdown showing Carry as the dominant action at 61.8\%.}
\end{figure}

The screenshot illustrates a representative inference scenario. The ball carrier is placed at approximately $(83, 40)$, roughly 37 yards from goal. Two outfield teammates are positioned at the near edge of the penalty area (one upper, one lower), two opponents are inside the box, and the opposing goalkeeper is on the goal line. The model returns a weighted xT of \textbf{0.372} with no pressure detected.

The action breakdown reveals the model's reasoning explicitly. \textbf{Carry} dominates with a 61.8\% weight and a contribution of 0.193, because no defenders are within 10 yards of the ball carrier; the carry weight formula $\max(0, 1 - n_{\mathrm{close}}/4)$ evaluates to approximately 1.0, reflecting that carrying toward goal from this position is uncontested. \textbf{Pass} accounts for 24.7\% of the weight: two available teammates give a pass weight of $2/5 = 0.40$ before normalisation, and the best pass target is identified as the upper teammate at $(97.7, 22)$, who is in a dangerous position at the box edge (xT 0.243). \textbf{Shot} receives a 13.4\% weight and a high individual xT of 0.883 (reflecting genuine danger if a shot is attempted), but the weight is suppressed by the large distance, with $e^{-37/25} \approx 0.23$, and partial shooting-lane obstruction from the opponents inside the box. \textbf{Dribble} receives 0\% weight because no defender is within 5 yards, so the dribble trigger condition is not met.

The weighted total is $0.134 \times 0.883 + 0.247 \times 0.243 + 0.618 \times 0.312 + 0 \approx 0.372$, matching the displayed value exactly. The interface makes this decomposition immediately legible: not just a headline xT number, but a transparent account of which action types drive the prediction and why, allowing users to understand intuitively how player positioning shapes the model's assessment of attacking threat.
...
Each version is a direct response to what the previous one could not do, and the metric progression reflects that.
